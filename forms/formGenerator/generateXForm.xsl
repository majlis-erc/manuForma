        ...
        <html xmlns="http://www.w3.org/1999/xhtml" 
            ...
            <head>
                ...
                <script type="text/javascript">
                    /*
                    function showHidElement(elementID) {
                    ...
                    */
                    /*
                    $('.showLookup a').on('click', function(e){ // on change of state
                        e.preventDefault(e);
                        var current = $(this) 
                        var $annotationsResults = $(current).closest('.lookupDiv');
                            //If annotation results are empty load via ajax, otherwise toggle to show or hide div
                            if($annotationsResults.style.display === "none"){
                                $annotationsResults.style.display = "block"; 
                            } else {
                                $annotationsResults.style.display = "none";
                        }   
                    });
                    */
                </script>
                ...
                <div class="d-flex flex-column justify-content-center align-items-center">
                    ...
                    <div class="input-group">    
                        ...
                        <xf:select1 ref="instance('i-lookup-selected')/child::*" appearance="full" class="checkbox select-group">
                            <xf:itemset ref="instance('i-lookup-results')//*:persName">
                                <xf:label ref="."/>
                                <xf:value ref="."/>
                            </xf:itemset>
                            <xf:action ev:event="xforms-value-changed">
                                <xf:insert ev:event="DOMActivate" 
                                    ...
                                    origin="instance('i-lookup-results')//*:persName[. = instance('i-lookup-selected')//selected]"/>
                            </xf:action>
                        </xf:select1>
...
<!--                            <message level="modal" ev:event="DOMActivate">Added</message>-->
    ...
    <xsl:template name="xformElementUI">
        ...
        <div class="btn-toolbar justify-content-between mt-3 elementControls" role="toolbar">
            <div class="btn-group" role="group">
                ...
                <span class="elementLabel">
                    <xf:output value="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/@elementLabel"/>
                </span>
            </div>
            <div class="input-group">
                <xf:select1 xmlns="http://www.w3.org/2002/xforms" ref="instance('i-availableElements')/*[local-name() = local-name(current())][instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/*:availableAtts/descendant-or-self::*:attDef]">
                    <xf:label></xf:label>
                    <xf:itemset ref="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/*:availableAtts/descendant-or-self::*:attDef">
                        <xf:label ref="@ident"/>
                        <xf:value ref="@ident"/>
                    </xf:itemset>
                    <xf:action ev:event="xforms-value-changed">
                ...
                <xf:trigger class="btn btn-outline-secondary btn-sm controls add" appearance="full" ref=".[instance('i-availableElements')/*[local-name() = local-name(current())][instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/*:availableAtts/descendant-or-self::*:attDef]]">
                ...
                <xf:select1 xmlns="http://www.w3.org/2002/xforms" ref="instance('i-availableElements')/*[local-name() = local-name(current())][instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/*:childElements[1]/*:child/*:element]">
                    <xf:label></xf:label>
                    <xf:item>
                        <xf:label> - select - </xf:label>
                        <xf:value/>
                    </xf:item>
                    <xf:itemset ref="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/*:childElements[1]/*:child/*:element">
                        <xf:label ref="@ident"/>
                        <xf:value ref="@ident"/>
                    </xf:itemset>
                    <xf:action ev:event="xforms-value-changed">                            
                ...
                <xf:trigger appearance="minimal" 
                    ref=".[instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/*:lookup]" 
                    ...
                    <xf:action ev:event="DOMActivate">
                        ...
                        <xf:load show="embed" targetid="{$repeatID}subformLookup">
                            <xf:resource value="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/*:lookup/@formURL"/>
                        </xf:load>
                    </xf:action>
                </xf:trigger>
        ...
        <div class="element">   
            <div class="alert alert-light hint elementDesc">
                <xf:output ref="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/*:desc"/>                    
            </div>
            ...
            <span class="element-group">                        
                <xf:input ref=".[(instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/*:childElements/*:textNode[@type='input']) or (not(child::*) and (instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/*:childElements/*:textNode[@type='input']))]"></xf:input>
                <xf:textarea ref=".[instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/*:childElements/*:textNode[@type='textarea']]"></xf:textarea>  
    ...
    <xsl:template name="xformElementSchemaInstance">
        ...
        <xsl:element name="{$elementName}" namespace="http://www.tei-c.org/ns/1.0">
            ...
            <xsl:copy-of select="$subform/*:popupForm[@elementName = $elementName]"/>
