...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:sc="http://www.ascc.net/xml/schematron" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:xi="http://www.w3.org/2001/XInclude" xmlns:tei="http://www.tei-c.org/ns/1.0"
    xmlns:srophe="https://srophe.app" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xf="http://www.w3.org/2002/xforms"
    ...
    <xsl:function name="local:childElements">
        ...
        <xsl:variable name="elementRefs">
            ...
            <xsl:for-each-group
                select="$rules/descendant-or-self::tei:content/descendant-or-self::*[@key] | $rules/descendant-or-self::tei:alternate/descendant-or-self::*[@key]"
                group-by="@key">
                <xsl:choose>
                    <xsl:when
                        test="(current-grouping-key() = ('macro.specialPara', 'model.pLike')) or (contains(current-grouping-key(), 'macro.specialPara') or contains(current-grouping-key(), 'model.pLike'))">
                        <element xmlns="http://www.tei-c.org/ns/1.0" ident="p"/>
                    </xsl:when>
                    <xsl:when
                        test="(current-grouping-key() = ('model.gLike', 'macro.xtext', 'macro.phraseSeq')) or (contains(current-grouping-key(), 'macro.phraseSeq') or contains(current-grouping-key(), 'macro.xtext') or contains(current-grouping-key(), 'model.gLike'))"/>
    ...
    <xsl:function name="local:resolveMacroSpec">
        ...
        <xsl:variable name="localRules">
            <xsl:for-each
                select="$globalSchemaDoc/descendant-or-self::tei:macroSpec[@ident = $macroSpec]/descendant-or-self::tei:content/descendant-or-self::*[@key]">
                <xsl:choose>
                    <xsl:when test=".[@mode = 'delete']">
    ...
    <xsl:function name="local:allAttributes">
        ...
        <xsl:variable name="memberOfAtt">
            <xsl:for-each-group select="$elementRules/descendant-or-self::tei:classes/tei:memberOf[starts-with(@key, 'att.')]" group-by="@key">
                <xsl:choose>
                    ...
                    <xsl:when test=".[@mode = 'change'][ancestor-or-self::tei:local]">
                        <xsl:copy-of select="local:classSpecAtt(.[@mode = 'change'][ancestor-or-self::tei:local][1]/@key, $subform)"/>
                    </xsl:when>
                    <xsl:when test=".[@mode = 'opt'][ancestor-or-self::tei:local]">
                        <xsl:copy-of select="local:classSpecAtt(.[@mode = 'opt'][ancestor-or-self::tei:local][1]/@key, $subform)"/>
                    </xsl:when>
                    <xsl:when test=".[@mode = 'add'][ancestor-or-self::tei:local]">
                        <xsl:copy-of select="local:classSpecAtt(.[@mode = 'add'][ancestor-or-self::tei:local][1]/@key, $subform)"/>
                    </xsl:when>
                    <xsl:when test=".[@mode = 'replace'][ancestor-or-self::tei:local]">
                        <xsl:copy-of select="local:classSpecAtt(.[@mode = 'replace'][ancestor-or-self::tei:local][1]/@key, $subform)"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:copy-of select="local:classSpecAtt(@key, $subform)"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:for-each-group>
        </xsl:variable>
        <availableAtts xmlns="http://www.tei-c.org/ns/1.0" elementName="{$elementName}">
            <xsl:for-each-group
                select="$childAtt/descendant-or-self::tei:attDef | $childAttRef/descendant-or-self::tei:attDef | $memberOfAtt/descendant-or-self::tei:attDef"
                group-by="@ident">
                <xsl:sort select="current-grouping-key()"/>
                <xsl:choose>
                    <xsl:when test=".[@mode = 'delete'][ancestor-or-self::tei:local]"/>
                    <xsl:when test=".[@mode = 'change'][ancestor-or-self::tei:local]">
                        <xsl:copy-of select=".[@mode = 'change'][ancestor-or-self::tei:local][1]"/>
                    </xsl:when>
                    <xsl:when test=".[@mode = 'opt'][ancestor-or-self::tei:local]">
                        <xsl:copy-of select=".[@mode = 'opt'][ancestor-or-self::tei:local][1]"/>
                    </xsl:when>
                    <xsl:when test=".[@mode = 'add'][ancestor-or-self::tei:local]">
                        <xsl:copy-of select=".[@mode = 'add'][ancestor-or-self::tei:local][1]"/>
                    </xsl:when>
                    <xsl:when test=".[@mode = 'replace'][ancestor-or-self::tei:local]">
                        <xsl:copy-of select=".[@mode = 'replace'][ancestor-or-self::tei:local][1]"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:copy-of select="."/>
                    </xsl:otherwise>
    ...
    <xsl:template match="/">
        ...
        <xsl:for-each select="$configDoc//subforms/subform">
            ...
            <xsl:variable name="maxLevel">
                <xsl:choose>
                    <xsl:when test="$levels castable as xs:integer">
                        <xsl:value-of select="xs:integer($levels)"/>
                    </xsl:when>
        ...
        <html xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
            <head>
                ...
                <xf:model id="m-mss">
                    ...
                    <xsl:if test="$configDoc//*:projectSpecificData/*:xmlPath[@src != '']">
                        ...
                        <xsl:variable name="src">
                            <xsl:choose>
                                ...
                                <xsl:otherwise>
                                    <xsl:value-of select="string($configDoc//*:projectSpecificData/*:template/@src)"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </xsl:variable>
                    ...
                    <xf:submission id="s-load-template" method="post" ref="instance('i-selected')" replace="instance" instance="i-rec"
                        ...
                        <xf:action ev:event="xforms-submit-done">
                            <xf:message level="modeless"> Record loaded! </xf:message>
                            <xf:insert ref="instance('i-rec')//*:titleStmt/child::*" at="last()" origin="instance('i-admin')//*:editor"
                                position="after"/>
                            ...
                            <xf:insert ref="instance('i-rec')//*:revisionDesc/child::*" at="last()" origin="instance('i-admin')//*:change[1]"
                                position="after"/>
                            <xf:setvalue ref="instance('i-rec')//*:revisionDesc/*:change[last()]"
                                value="concat('Record created by manuForma, editor: ', instance('i-user')//*:fullName)"/>
                    ...
                    <xf:submission id="s-load-template-search" method="post" ref="instance('i-selected-search')" replace="instance" instance="i-rec"
                        serialization="none" mode="synchronous">
                        <xf:resource value="concat('services/get-rec.xql?template=true&amp;path=',instance('i-selected-search'))"/>
                        <xf:message level="modeless" ev:event="xforms-submit-done"> Record loaded! </xf:message>
                        <xf:message level="modeless" ev:event="xforms-submit-error"> Submit error. </xf:message>
                    </xf:submission>

                    <xf:submission id="s-search-saved" method="post" ref="instance('i-search')" replace="instance" instance="i-search-results"
                        serialization="none" mode="synchronous">
                        <xf:resource
                            value="concat('services/get-rec.xql?search=true&amp;q=',instance('i-search'),'&amp;eXistCollection=',string(instance('i-submission-params')//*:retrieveOptions/*:option[@name='exist-db']/*:parameter[@name='eXistCollection']))"/>
                        <xf:message level="modeless" ev:event="xforms-submit-error"> Submit error. </xf:message>
                    </xf:submission>

                    <xf:submission id="s-browse-saved" method="post" ref="instance('i-search')" replace="instance" instance="i-search-results"
                        serialization="none" mode="synchronous">
                        <xf:resource
                            value="concat('services/get-rec.xql?search=true&amp;view=all&amp;eXistCollection=',string(instance('i-submission-params')//*:retrieveOptions/*:option[@name='exist-db']/*:parameter[@name='eXistCollection']))"/>
                        <xf:message level="modeless" ev:event="xforms-submit-error"> Submit error. </xf:message>
                    </xf:submission>
                    <xf:submission id="s-post-to-update" action="services/upload.xql" ref="instance('i-upload')" replace="instance" instance="i-rec"
                        method="post">
                        <xf:message level="modeless" ev:event="xforms-submit-done"> Record Loaded! </xf:message>
                        <xf:message level="modeless" ev:event="xforms-submit-error"> Submit error. </xf:message>
                    </xf:submission>
                    ...
                    <xf:submission id="s-download-xml" ref="instance('i-rec')" show="new" method="post" replace="all"
                        action="form.xq?form=services/download.xml?type=download"/>
                    <!-- Download View XML in browser -->
                    <xf:submission id="s-view-xml" ref="instance('i-rec')" show="new" method="post" replace="instance" instance="i-submission"
                        action="services/submit.xql?type=view"/>
                    <xf:submission id="s-view-html" ref="instance('i-rec')" show="new" replace="all" instance="i-submission" method="post"
                        action="services/submit.xql?type=previewHTML"/>
                    <!-- Save record to Database  -->
                    <xf:submission id="s-save" ref="instance('i-rec')" replace="instance" instance="i-submission" method="post">
                        <xf:resource
                            value="concat('services/submit.xql?type=save&amp;eXistCollection=',string(instance('i-submission-params')//*:saveOptions/*:option[@name='exist-db']/*:parameter[@name='eXistCollection']))"/>
                        <xf:action ev:event="xforms-submit-done">
                    ...
                    <div class="row">
                            ...
                            <div class="position-sticky pt-3">
                                <ul class="nav flex-column">
                                    ...
                                    <xsl:for-each select="$configDoc//subforms/subform[not(@formLabel = 'Header')]">
                                        <xsl:variable name="subformPath"
                                            select="replace(replace(@xpath, 'descendant-or-self::', '/'), 'descendant::', '/')"/>
                                        <xsl:variable name="elementName" select="substring-after(tokenize(@xpath, '/')[last()], ':')"/>
                                        <xsl:variable name="path"
                                            select="replace(replace(replace(replace(replace($subformPath, 'Q\{http://www.tei-c.org/ns/1.0\}', '*:'), 'tei:TEI', '/'), '\[[0-9]+\]', ''), '///', '//'), 'tei:', '*:')"/>
                                        <xsl:variable name="repeatIndexId" select="concat('navRepeat', @formName)"/>
                                        <li class="nav-item">
                                            <xf:trigger appearance="minimal"
                                                ref="instance('i-subforms')//*:subform[@formName = '{string(@formName)}']" class="nav-link">
                                                <xf:label>
                                                    <xsl:value-of select="string(@formLabel)"/>
                                                </xf:label>
                                                <!--
                                                <xf:action
                                                ...
                                                <xf:action ev:event="DOMActivate">
                                                    ...
                                                    <xf:load if="@selected != 'true'" show="embed" targetid="subform"
                                                        resource="{concat('form.xq?form=forms/',$mainFormName,'/',@formName,'.xhtml')}"/>
                                        ...
                                        <xf:repeat ref="instance('i-rec'){replace($path,'///','//')}[position() &gt; 1]" id="{$repeatIndexId}">
                                            <li class="nav-item">
                                                <xf:trigger appearance="minimal" class="nav-link">
                                                    <xf:label><xsl:value-of select="string(@formLabel)"/> [<xf:output value="position() + 1"
                                                        />]</xf:label>
                                                    <xf:action ev:event="DOMActivate">
                                                        <xf:toggle case="view-data-entry"/>
                                                        <xf:unload targetid="subform"/>
                                                        <xf:setvalue ref="instance('i-subforms')//*:subform[@selected = 'true']/@selected"
                                                            value="'false'"/>
                                                        <xf:load if="@selected != 'true'" show="embed" targetid="subform"
                                                            resource="{concat('form.xq?form=forms/',$mainFormName,'/',@formName,'.xhtml')}"/>
                                                        <xf:setvalue ref="@selected" value="'true'"/>
                                                        <xsl:element name="setvalue" namespace="http://www.w3.org/2002/xforms">
                                                            <xsl:attribute name="ref">instance('i-repeatIndex')/index</xsl:attribute>
                                                            <xsl:attribute name="value">index('<xsl:value-of select="$repeatIndexId"/>') +
                                                                1</xsl:attribute>
                                                        </xsl:element>
                                                    </xf:action>
                                                </xf:trigger>
                        ...
                        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 border-bottom">
                                <h1 class="h2"><!--<xsl:value-of select="$configDoc//formTitle"/>--> Edit tecord: <xf:output
                                        value="instance('i-rec')//*:titleStmt/*:title[@level='a']" class="elementLabel"/>
                                </h1>
                                    ...
                                    <div class="btn-group me-2">
                                        <div class="submission float-end">
                                            ...
                                            <xsl:for-each select="$configDoc//subforms/subform[@formLabel = 'Header']">
                                                ...
                                                <xsl:variable name="repeatIndexId" select="concat('navRepeat', @formName)"/>
                                                <xf:trigger appearance="minimal"
                                                    ref="instance('i-subforms')//*:subform[@formName = '{string(@formName)}']"
                            ...
                            <xsl:if test="$configDoc//formDesc != ''">
                                <p class="alert alert-light hint">
                                    <xsl:value-of select="$configDoc/config/formDesc[1]"/>
                                </p>
                            </xsl:if>
                            <xf:switch id="edit" class="mainContent panel">
                                    ...
                                    <div class="accordion" id="loadRecords">
                                        <div class="accordion-item">
                                            <h2 class="accordion-header" id="headingProjectMetadata">
                                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                                    data-bs-target="#collapseProjectMetadata" aria-expanded="false"
                                                    aria-controls="collapseProjectMetadata"> Assign project metadata </button>
                                            </h2>
                                            <div id="collapseProjectMetadata" class="accordion-collapse collapse"
                                                aria-labelledby="headingProjectMetadata" data-bs-parent="#loadRecords">
                                                    ...
                                                    <xsl:if test="$configDoc//*:projectSpecificData/*:xmlPath[@src != '']">
                                                        ...
                                                        <xf:trigger class="btn btn-outline-secondary btn-sm controls add" appearance="full">
                                                            ...
                                                            <xf:delete ev:event="DOMActivate"
                                                                ref="instance('i-rec')//*:{$configDoc//*:projectSpecificData/*:xmlPath/@element}"/>
                                                            <xf:insert ev:event="DOMActivate" ref="instance('i-rec')//*:titleStmt" at="."
                                                                origin="instance('i-projectSpecificData')//*:option[@name = instance('i-projectSpecificDataSelected')//*:selected]//*:{$configDoc//*:projectSpecificData/*:xmlPath/@element}"
                                                                position="after"/>
                                        ...
                                        <div class="accordion-item">
                                            <h2 class="accordion-header" id="headingExistingRecord">
                                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                                    data-bs-target="#collapseExistingRecord" aria-expanded="false"
                                                    aria-controls="collapseExistingRecord"> Continue work with existing record </button>
                                            </h2>
                                            <div id="collapseExistingRecord" class="accordion-collapse collapse"
                                                aria-labelledby="headingExistingRecord" data-bs-parent="#loadRecords">
                                                <div class="accordion-body">
                                                    <div class="fileLoading">
                                                        <div class="input-group mb-3">
                                                            ...
                                                            <xf:submit class="btn btn-outline-secondary" submission="s-search-saved"
                                                                appearance="minimal">
                                                                <xf:label> Search </xf:label>
                                                            </xf:submit>
                                                            <xf:submit class="btn btn-outline-secondary" submission="s-browse-saved"
                                                                appearance="minimal">
                                                                <xf:label> Browse </xf:label>
                                                            </xf:submit>
                                                        </div>
                                                        <div class="input-group mb-3">
                                                            <xf:select1 xmlns="http://www.w3.org/2002/xforms" appearance="full"
                                                                class="checkbox select-group form-control" ref="instance('i-selected-search')">
                                                                <xf:label/>
                                                            ...
                                                            <xf:submit class="btn btn-outline-secondary" submission="s-load-template-search"
                                                                appearance="minimal">
                                        ...
                                        <div class="accordion-item">
                                            <h2 class="accordion-header" id="headingNewRecord">
                                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                                    data-bs-target="#collapseNewRecord" aria-expanded="false" aria-controls="collapseNewRecord"> Upload record </button>
                                            </h2>
                                            <div id="collapseNewRecord" class="accordion-collapse collapse" aria-labelledby="headingNewRecord"
                                                data-bs-parent="#loadRecords">
                                                <div class="accordion-body">
                                                    <div class="fileLoading">
                                                        <h4 class="h6">Upload record from your computer</h4>
                                                        <div class="input-group mb-3">
                                                            <xf:upload class="form-control" ref="instance('i-upload')" appearance="minimal">
                                                                <xf:label/>
                                                            </xf:upload>
                                                            <!-- WS:Note add upload graphic, make inline with above? -->
                                                            <xf:submit submission="s-post-to-update" class="btn btn-outline-secondary"
                                                                appearance="minimal">
                                                    ...
                                                    <!--<div class="fileLoading">
                                                        <div class="input-group mb-3">
                                                            <xf:select1 xmlns="http://www.w3.org/2002/xforms" class="form-control"
                                                                ref="instance('i-selected')">
                                                                <xf:label/>
                                                            ...
                                                            <xf:submit class="btn btn-outline-secondary" submission="s-load-template"
                                                                appearance="minimal">
                                ...
                                <xf:case id="view-admin">
                                    ...
                                    <xf:repeat ref="instance('i-rec')//*:revisionDesc/*:change">
                                        ...
                                        <div class="input-group mb-3">
                                            <xf:input ref="@when" class="form-control">
                                    ...
                                    <div class="input-group mb-3">
                                        <xf:input ref="instance('i-rec')//*:publicationStmt[1]/*:idno[@type='URI']" class="form-control">
                                            <xf:label>URI (Don't change this unless you know what you are doing): </xf:label>
    ...
    <xsl:template name="xform">
        ...
        <xsl:variable name="path"
            select="replace(replace(replace(replace(replace($xpath, 'Q\{http://www.tei-c.org/ns/1.0\}', '*:'), 'tei:TEI', '/'), '\[[0-9]+\]', ''), '///', '//'), 'tei:', '*:')"/>
        <!-- 
        ...
        <html>
            <head>
                ...
                <xf:model id="m-mss">
                    ...
                    <xf:instance id="i-{string($subformName)}-schemaConstraints"
                        src="forms/{$mainFormName}/templates/{$subformName}-schemaConstraints.xml"/>
                    <xf:instance id="i-{string($subformName)}-elementTemplate"
                        src="forms/{$mainFormName}/templates/{$subformName}-elementTemplate.xml"/>
            ...
            <body>
                ...
                <xsl:variable name="elementPath"
                    select="replace(replace(replace(replace($xpath, 'Q\{http://www.tei-c.org/ns/1.0\}', '*:'), 'tei:TEI', '/'), '\[[0-9]+\]', ''), '///', '//')"/>
                ...
                <xsl:if test="$subform/formDesc">
                    <p class="alert alert-light hint">
                        <xsl:value-of select="$subform/formDesc"/>
                    </p>
                </xsl:if>
                <xf:group namespace="http://www.w3.org/2002/xforms"
                    ref="instance('i-rec'){replace($path,'///','//')}[position() = instance('{$repeatIndex}')/index]" id="{$elementName}-root">
                    <xsl:choose>
                                        ...
                                        <div class="position-sticky pt-3">
                                            <ul class="nav flex-column">
                                                <xsl:for-each select="subform">
                                                    ...
                                                    <li class="nav-item">
                                                        <xf:trigger appearance="minimal" class="nav-link">
                                                            <xsl:attribute name="ref">instance('<xsl:value-of
                                                                    select="concat('i-', $subformName, '-subforms')"/>')//*:subform[@formName =
                                                                    '<xsl:value-of select="string(@formName)"/>']</xsl:attribute>
                                                            <xf:label>
                                                                <xsl:value-of select="string(@formLabel)"/>
                                                            </xf:label>
                                                            <xf:action ev:event="DOMActivate">
                                                                ...
                                                                <xf:setvalue value="'false'">
                                                                    <xsl:attribute name="ref">instance('<xsl:value-of
                                                                           select="concat('i-', $subformName, '-subforms')"/>')//*:subform[@selected =
                                                                        'true']/@selected</xsl:attribute>
                                                                </xf:setvalue>
                                                                <xf:load if="@selected != 'true'" show="embed" targetid="{$subformName}Subform"
                                                                    resource="{concat('form.xq?form=forms/',$mainFormName,'/',$subformName,'/',@formName,'.xhtml')}"/>
                                                                <xf:setvalue ref="@selected" value="'true'"/>
                                                                <xsl:element name="setvalue" namespace="http://www.w3.org/2002/xforms">
                                                                    <xsl:attribute name="ref">instance('<xsl:value-of
                                                                           select="concat('i-', $subformName, '-repeatIndex')"
                                                                        />')/index</xsl:attribute>
                                                                    <xsl:attribute name="value">1</xsl:attribute>
                                                                </xsl:element>
                                                            </xf:action>
                        ...
                        <xsl:otherwise>
                            <xsl:call-template name="xformElementUI">
                                ...
                                <xsl:with-param name="xpathIndex"><xsl:value-of select="$elementPath"/>[position() =
                                    instance('i-repeatIndex')/index]</xsl:with-param>
        ...
        <html>
            ...
            <body>
                <div class="d-flex flex-column justify-content-center align-items-center">
                    <h2 class="h3 mainElement">
                        <xsl:value-of select="$lookup/@lookupLabel"/>
                    </h2>
                    ...
                    <xsl:choose>
                        <xsl:when test="$lookup/@elementName = 'bibl'">
                            ...
                            <xf:select1 ref="instance('i-lookup-selected')/child::*" appearance="full" class="checkbox select-group lookup-results">
                                ...
                                <xf:action ev:event="xforms-value-changed">
                                    <xf:insert ev:event="DOMActivate" ref="instance('i-lookup-add')/child::*"
                            ...
                            <xf:trigger class="btn btn-outline-secondary btn-sm" appearance="full" ref=".">
                                <xf:label><i class="bi bi-plus-circle"/> Add to record </xf:label>
                                <xf:insert ev:event="DOMActivate" ref="." at="." origin="instance('i-lookup-selected')/selected/child::*/child::*[1]"
                                    position="after"/>
                                <xf:delete ev:event="DOMActivate" ref="."/>
                        ...
                        <xsl:otherwise>
                            ...
                            <xf:trigger class="btn btn-outline-secondary btn-sm" appearance="full" ref=".">
                                <xf:label><i class="bi bi-plus-circle"/> Add to record</xf:label>
                                <xf:insert ev:event="DOMActivate" ref="." at="." origin="instance('i-lookup-selected')/selected/child::*"
                                    position="after"/>
                                <xf:delete ev:event="DOMActivate" ref="."/>
                                <xf:setvalue ref="instance('i-lookup-query')/q" value="''"/>
                                <xf:delete ref="instance('i-lookup-results')//child::*"/>
    ...
    <xsl:template name="xformElementUI">
        ...
        <xsl:variable name="elementPath"
            select="replace(replace(replace(replace($subform/@xpath, 'Q\{http://www.tei-c.org/ns/1.0\}', '*:'), 'tei:TEI', '/'), '\[[0-9]+\]', ''), '///', '//')"/>
        <xsl:variable name="xpathIndex">
            <xsl:choose>
                <xsl:when test="$subform/parent::*:subform and $currentLevel = 1">
                    ...
                    <xsl:variable name="parentXPathCleaned"
                        select="replace(replace(replace(replace(replace($parentXPath, 'Q\{http://www.tei-c.org/ns/1.0\}', '*:'), 'tei:TEI', '/'), '\[[0-9]+\]', ''), '///', '//'), 'tei:', '*:')"/>
                    <xsl:value-of select="$parentXPathCleaned"/>
                    <xsl:text>[position() = instance('i-repeatIndex')/index]/</xsl:text>
                    <xsl:value-of select="replace($elementPath, 'tei:', '*:')"/>
                    <xsl:text>[position() = instance('i-</xsl:text>
                    <xsl:value-of select="string($subformName)"/>
                    <xsl:text>-repeatIndex')/index]/*</xsl:text>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$xpathIndex"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <!-- Build XForm element -->
        <xsl:choose>
                        ...
                        <xsl:variable name="grpRepeatID">
                            <xsl:choose>
                                <xsl:when test="$subform/parent::*:subform">
                                    <xsl:value-of
                                        select="
                                            concat(string($subform/parent::*:subform/@formName), 'Subform', $subformName, 'GrpRepeatLevel', if (string($currentLevel) != '') then
                                                string($currentLevel)
                                            else
                                                '1')"
                                    />
                                ...
                                <xsl:otherwise>
                                    <xsl:value-of
                                        select="
                                            concat($subformName, 'GrpRepeatLevel', position(), if (string($currentLevel) != '') then
                                                string($currentLevel)
                                            else
                                                '1')"
                                    />
                        ...
                        <xsl:variable name="groupElements">
                            <xsl:variable name="elements">
                                <xsl:for-each select="element">
                                    <xsl:value-of select="replace(@xpath, 'tei:', 'self::*:')"/>
                                ...
                                <xsl:choose>
                                    <xsl:when test="contains($groupElements, '/')">
                                        <xsl:variable name="parentElements">
                                            <xsl:variable name="elements">
                                                <xsl:for-each select="element">
                                                    <xsl:value-of select="replace(substring-before(@xpath, '/'), 'tei:', 'self::*:')"/>
                                                    <xsl:if test="position() != last()">
                                                        <xsl:text> | </xsl:text>
                                                    </xsl:if>
                                                </xsl:for-each>
                                        ...
                                        <xsl:variable name="childElements">
                                            <xsl:variable name="elements">
                                                <xsl:for-each select="element">
                                                    <xsl:value-of select="replace(substring-after(@xpath, '/'), 'tei:', 'self::*:')"/>
                                                    <xsl:if test="position() != last()">
                                                        <xsl:text> | </xsl:text>
                                                    </xsl:if>
                                                </xsl:for-each>
                            ...
                            <xsl:otherwise>
                                <div class="accordion-item">
                                    <h2 class="accordion-header" id="heading{@groupNo}">
                                        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                            data-bs-target="#collapse{@groupNo}" aria-expanded="false" aria-controls="collapse{@groupNo}">
                                            <xsl:choose>
                                    ...
                                    <div id="collapse{@groupNo}" class="accordion-collapse collapse" aria-labelledby="heading{@groupNo}"
                                        ...
                                        <div class="accordion-body">
                                            ...
                                            <div xmlns="http://www.w3.org/1999/xhtml" class="btn-group" role="group"
                                                ...
                                                <div class="input-group mb-3 float-end">
                                                    ...
                                                    <xf:select1 xmlns="http://www.w3.org/2002/xforms" class="addElementsGrp"
                                                        ref="instance('i-availableElements')/*[local-name() = local-name(current())][instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:childElements[1]/*:child/*:element]">
                                                        <xf:label/>
                                                        <xsl:for-each select="element">
                                                            <xf:item xmlns="http://www.w3.org/2002/xforms">
                                                                <xf:label
                                                                    ref="instance('i-{$subformName}-schemaConstraints')/*[local-name() = '{replace(@xpath,'tei:','')}']/@elementLabel"/>
                                                                <xf:value>
                                                                    <xsl:value-of select="replace(@xpath, 'tei:', '')"/>
                                                                </xf:value>
                                                            </xf:item>
                                                        </xsl:for-each>
                                                        <xf:action ev:event="xforms-value-changed">
                                                    ...
                                                    <xf:trigger xmlns="http://www.w3.org/2002/xforms"
                                                        class="btn btn-outline-secondary btn-sm controls add" appearance="minimal">
                                                        <xf:label><i class="bi bi-plus-circle"/> Add field</xf:label>
                                                        <!-- Add conditions here -->
                                                        <xf:insert ev:event="DOMActivate" context="." at="."
                                                            origin="instance('i-{$subformName}-elementTemplate')/*[local-name() = instance('i-insert-elements')//*:element][1]"
                                                            position="after"/>
                                                        <xf:setvalue ev:event="DOMActivate" ref="instance('i-insert-elements')//*:element"/>
                                                        <xf:setvalue ev:event="DOMActivate"
                                                            ref="instance('i-availableElements')/*[local-name() = local-name(current())][instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/*:childElements[1]/*:child/*:element]"
                                                        />
                                                    </xf:trigger>
                                                </div>
                                            </div>
                                            <xsl:choose>
                                                <xsl:when test="contains($groupElements, '/')">
                                                    ...
                                                    <xf:repeat xmlns="http://www.w3.org/2002/xforms" id="{$grpRepeatID}a" ref="./*[{$parentElements}]">
                                                        <xf:repeat xmlns="http://www.w3.org/2002/xforms" id="{$grpRepeatID}"
                                                            ref="./*[{$childElements}]">
            ...
            <xsl:otherwise>
                ...
                <xf:group xmlns="http://www.w3.org/2002/xforms"
                    ref=".[instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]]">
                        ...
                        <div class="accordion-body">
                            <div xmlns="http://www.w3.org/1999/xhtml" class="btn-group" role="group" style="width:100%; border-bottom:1px solid #ccc;">
                                <div class="input-group mb-3 float-end">
                                    <xf:trigger class="btn controls add" appearance="minimal">
                                        ...
                                        <xf:insert ev:event="DOMActivate" context="parent::*"
                                            origin="instance('i-{$subformName}-elementTemplate')/*[local-name() = '{replace($elementName,'tei:','')}']"
                                            position="after"/>
                                    </xf:trigger>
                                    <xsl:value-of select="@groupLabel"/>
                                </div>
                            ...
                            <xsl:variable name="childRepeatID">
                                <xsl:choose>
                                    <xsl:when test="$grpRepeatID">
                                        <xsl:value-of
                                            select="
                                                concat($grpRepeatID, 'RepeatLevel', position(), 'child', if (string($currentLevel) != '') then
                                                    string($currentLevel)
                                                else
                                                    '1')"
                                        />
                                    ...
                                    <xsl:when test="$repeatID != ''">
                                        <xsl:choose>
                                            <xsl:when test="$subform/parent::*:subform">
                                                <xsl:value-of
                                                    select="
                                                        concat($repeatID, 'RepeatLevel', if (string($currentLevel) != '') then
                                                            string($currentLevel)
                                                        else
                                                            '1')"
                                                />
                                    ...
                                    <xsl:when test="$subform/parent::*:subform">
                                        <xsl:value-of
                                            select="
                                                concat(string($subform/parent::*:subform/@formName), 'Subform', $subformName, 'RepeatLevel', if (string($currentLevel) != '') then
                                                    string($currentLevel)
                                                else
                                                    '1')"
                                        />
                                    ...
                                    <xsl:otherwise>
                                        <xsl:value-of
                                            select="
                                                concat($subformName, 'RepeatLevel', if (string($currentLevel) != '') then
                                                    string($currentLevel)
                                                else
                                                    '1')"
                                        />
                            ...
                            <div class="element">
                                <div class="inlineDisplay btn-toolbar justify-content-between" role="toolbar">
                                    ...
                                    <xf:select1
                                        ref=".[instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:controlledValues/*:element/*:valList/*:valItem]"
                                        class="elementSelect">
                                        <xf:itemset
                                            ref="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:controlledValues/*:element/*:valList/*:valItem">
                                            <xf:label ref="@ident"/>
                                    ...
                                    <!-- Element input -->
                                    <xf:input class="elementInput"
                                        ref=".[not(instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:controlledValues) and instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:childElements/*:textNode[@type='input']]"/>
                                    ...
                                    <xf:textarea class="elementTextArea"
                                        ref=".[instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:childElements/*:textNode[@type='textarea']]"/>
                                    <!-- Element attributes -->
                                    <xf:repeat xmlns="http://www.w3.org/2002/xforms" ref="@*" class="attr-group">
                                        <div xmlns="http://www.w3.org/1999/xhtml" class="btn-group" role="group">
                                            <div class="input-group">
                                                <!-- Attribute value -->
                                                <xf:input
                                                    ref=".[not(instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current()/parent::*)][1]//*:availableAtts/*:attDef[@ident = name(current())]//*:valList)]"
                                                    class="attVal"/>
                                                <!-- If controlled attribute values in schemaConstraints file -->
                                                <xf:select1
                                                    ref=".[instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current()/parent::*)][1]//*:availableAtts/*:attDef[@ident = name(current())]//*:valList]"
                                                    class="attVal">
                                                    <xf:itemset
                                                        ref="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current()/parent::*)][1]//*:availableAtts/*:attDef[@ident = name(current())]//*:valList/*:valItem"
                                                        class="attVal">
                                                        <xf:label ref="@ident"/>
                                                        <xf:value ref="@ident"/>
                                                    </xf:itemset>
                                                </xf:select1>
                                                <xf:trigger xmlns="http://www.w3.org/2002/xforms" class="btn btn-outline-secondary btn-sm controls"
                                                    appearance="full" ref=".">
                                                    <xf:label>
                                                        <i xmlns="http://www.w3.org/1999/xhtml" class="bi bi-x-circle"/>
                                                        <xf:output
                                                            value="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current()/parent::*)][1]//*:availableAtts/*:attDef[@ident = name(current())]/@attLabel"
                                                        />
                                                    </xf:label>
                                                    <xf:delete ev:event="DOMActivate" ref="."/>
                                                </xf:trigger>
                                ...
                                <xsl:if test="$currentLevel &lt;= $maxLevel">
                                    <xsl:variable name="childRepeatID">
                                        <xsl:value-of
                                            select="
                                                concat($grpRepeatID, 'GrpRepeatLevel', if (string($currentLevel) != '') then
                                                    string($currentLevel)
                                                else
                                                    '1')"
                                        />
                                    </xsl:variable>
                                    <xf:repeat id="{$childRepeatID}"
                                        ref="./*[instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/*:childElements/*:child]">
                                        <div class="nested">
                                            <xsl:call-template name="xformElementUI">
                                                <xsl:with-param name="subform" select="$subform"/>
                                                <xsl:with-param name="xpath" select="concat($xpath, '/*/*')"/>
                                                <xsl:with-param name="xpathIndex"><xsl:value-of select="replace($xpathIndex, 'tei:', '*:')"
                                                    />/*/*</xsl:with-param>
    ...
    <xsl:template name="groupElementDisplay">
        ...
        <div xmlns="http://www.w3.org/1999/xhtml" class="element">
            ...
            <!-- Element input -->
            <xf:input xmlns="http://www.w3.org/2002/xforms" class="elementInput"
                ref=".[not(instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:controlledValues) and instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:childElements/*:textNode[@type='input']]"/>
            <!--<xf:input class="elementInput" ref=".[(instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:childElements/*:textNode[@type='input']"/>-->
            <!-- Element input for textbox style input -->
            <xf:textarea xmlns="http://www.w3.org/2002/xforms" class="elementTextArea"
                ref=".[instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:childElements/*:textNode[@type='textarea']]"/>
            <!-- Element attributes -->
            <xf:repeat xmlns="http://www.w3.org/2002/xforms" ref="@*" class="attr-group">
                <div xmlns="http://www.w3.org/1999/xhtml" class="btn-group" role="group">
            ...
            <xsl:if test="$currentLevel &lt;= $maxLevel">
                ...
                <xf:repeat xmlns="http://www.w3.org/2002/xforms" id="{$childRepeatID}"
                    ref="./*[instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/*:childElements/*:child]"
                    class="nested">
                    <xsl:call-template name="xformElementUI">
                        <xsl:with-param name="subform" select="$subform"/>
    ...
    <xsl:template name="groupByButtonGrp">
        ...
        <div class="btn-toolbar justify-content-between mt-3 elementControls" role="toolbar" xmlns="http://www.w3.org/1999/xhtml">
            <div class="btn-group" role="group">
                <xf:trigger xmlns="http://www.w3.org/2002/xforms" appearance="minimal" class="btn controls remove inline"
                    ref=".[count(preceding-sibling::*[{$groupElements}]) != 0]">
                    <xf:label>
                        <i xmlns="http://www.w3.org/1999/xhtml" class="bi bi-x-circle"/>
                    </xf:label>
                    <xf:delete ev:event="DOMActivate" ref="."/>
                ...
                <xf:trigger xmlns="http://www.w3.org/2002/xforms" appearance="minimal" class="btn controls moveUp inline"
                    ...
                    <xf:action ev:event="DOMActivate">
                        ...
                        <xsl:element name="setvalue" namespace="http://www.w3.org/2002/xforms">
                            ...
                            <xsl:attribute name="value">count(context()/preceding-sibling::*[<xsl:value-of select="$groupElements"
                                />][1]/preceding-sibling::*) + 1</xsl:attribute>
                        </xsl:element>
                ...
                <xf:trigger xmlns="http://www.w3.org/2002/xforms" appearance="minimal" class="btn controls moveDown inline"
                    ref=".[count(following-sibling::*[{$groupElements}]) != 0]">
                    <xf:label>
                    ...
                    <xf:action ev:event="DOMActivate">
                        ...
                        <xsl:element name="setvalue" namespace="http://www.w3.org/2002/xforms">
                            <xsl:attribute name="ref">instance('i-move')/tmp</xsl:attribute>
                            <xsl:attribute name="value">count(context()/following-sibling::*[<xsl:value-of select="$groupElements"
                                />][1]/preceding-sibling::*) + 1</xsl:attribute>
                        </xsl:element>
                        <!-- Insert/copy current node -->
                        <xsl:element name="insert" namespace="http://www.w3.org/2002/xforms">
                ...
                <span class="elementLabel">
                    <xf:output value="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/@elementLabel"
                        class="elementLabel"/>
                    <a href="#" class="triggerTEITooltip">
                        <img src="resources/images/TEI-175.jpg"/>
                    </a>
                    <xf:output ref="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/@teiElement"
                        class="TEITooltip"/>
                    <a href="#" class="triggerElementTooltip">
                        <i class="bi bi-question-circle"/>
                    </a>
                    <xf:output ref="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:desc/text()"
                ...
                <div class="input-group float-end">
                    ...
                    -->
                    <xf:group
                        ref=".[not(count(@*[local-name()=instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:availableAtts//*:attDef/@ident]) = count(instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:availableAtts//*:attDef/@ident))]">
                        <xf:select1 xmlns="http://www.w3.org/2002/xforms"
                            ref="instance('i-availableElements')/*[local-name() = local-name(current())][instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:availableAtts//*:attDef]">
                            <xf:label/>
                            ...
                            <xf:itemset
                                ref="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:availableAtts/*:attDef">
                                <xf:label ref="@attLabel"/>
                                <xf:value ref="@ident"/>
                            </xf:itemset>
                            <xf:action ev:event="xforms-value-changed">
                                <xf:setvalue ref="instance('i-insert-attributes')//*:attribute"
                                    value="instance('i-availableElements')/*[local-name() = local-name(current())]"/>
                            </xf:action>
                        </xf:select1>
                        <xf:trigger class="btn btn-outline-secondary btn-sm controls add" appearance="full"
                            ref=".[instance('i-availableElements')/*[local-name() = local-name(current())][instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:availableAtts//*:attDef]]">
                            <xf:label><i class="bi bi-plus-circle"/> Add field</xf:label>
                            <xf:insert ev:event="DOMActivate" context=".[not(name(@*) = instance('i-insert-attributes')//*:attribute)]"
                                origin="instance('i-attributeTemplate')//@*[name(.) = instance('i-insert-attributes')//*:attribute]" position="after"/>
                            <xf:setvalue ref="instance('i-insert-attributes')//*:attribute" value="''"/>
                            <xf:setvalue ref="instance('i-availableElements')/*" value="''"/>
                        </xf:trigger>
                    </xf:group>
                    <xf:select1 xmlns="http://www.w3.org/2002/xforms"
                        ref="instance('i-availableElements')/*[local-name() = local-name(current())][instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:childElements[1]/descendant-or-self::*:element]">
                        <xf:label/>
                        ...
                        <xf:itemset
                            ref="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:childElements[1]/descendant-or-self::*:element">
                            <xf:label ref="@label"/>
                            <xf:value ref="@ident"/>
                        </xf:itemset>
                        <xf:action ev:event="xforms-value-changed">
                    ...
                    <xf:trigger class="btn btn-outline-secondary btn-sm controls add" appearance="full"
                        ref=".[instance('i-availableElements')/*[local-name() = local-name(current())][instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/*:childElements[1]/descendant-or-self::*:element]]">
                        <xf:label><i class="bi bi-plus-circle"/> Add field </xf:label>
                        <xf:insert ev:event="DOMActivate" context="." at="."
                            origin="instance('i-{$subformName}-elementTemplate')/*[local-name() = instance('i-insert-elements')//*:element][1]"
                            position="after"/>
                    ...
                    <xf:trigger appearance="minimal"
                        ...
                        <xf:label>
                            <i class="bi bi-search"/> Lookup </xf:label>
                        <xf:action ev:event="DOMActivate">
                            <xf:toggle case="{$repeatID}LookupUnHide" ev:event="DOMActivate"/>
                            <xf:load show="embed" targetid="{$repeatID}subformLookup">
                                <xf:resource
                                    value="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:lookup/@formURL"
                                />
                            </xf:load>
                        </xf:action>
                    </xf:trigger>
                    <xf:trigger appearance="minimal"
                        ...
                        class="btn btn-outline-secondary btn-sm controls add">
                        <xf:label>
                            <i class="bi bi-plus-circle"/> New <xf:output
                                value="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())]/@elementLabel"/>
                        </xf:label>
                        <xf:action ev:event="DOMActivate">
                            <xf:load show="new">
                                <xf:resource
                                    value="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current())][1]/*:popup/@formURL"
                                />
                            </xf:load>
                        </xf:action>
                    </xf:trigger>
                ...
                <xf:repeat xmlns="http://www.w3.org/2002/xforms" ref="@*" class="attr-group">
                    <div xmlns="http://www.w3.org/1999/xhtml" class="btn-group" role="group">
                        <div class="input-group">
                            ...
                            <xf:trigger xmlns="http://www.w3.org/2002/xforms" class="btn btn-outline-secondary btn-sm controls" appearance="full"
                                ref=".">
                                <xf:label>
                                    <i xmlns="http://www.w3.org/1999/xhtml" class="bi bi-x-circle"/>
                                    <xf:output
                    ...
                    <div class="nested">
                        <xsl:call-template name="xformElementUI">
                            ...
                            <xsl:with-param name="xpathIndex">
                                <xsl:choose>
                                    <xsl:when test="$repeatID != ''"><xsl:value-of select="replace($xpathIndex, 'tei:', '*:')"/>[index('<xsl:value-of
                                            select="$repeatID"/>')]/*</xsl:when>
                                    <xsl:when test="$subform/parent::*:subform and $currentLevel = 1">
            ...
            <nav class="navbar navbar-expand-md navbar-dark bg-dark">
                <div class="container-fluid">
                    <a class="navbar-brand col-md-3 col-lg-2 me-0 px-3" href="{$app-root}">
                        <xsl:value-of select="$configDoc//appTitle"/>
                    </a>
                    ...
                    <div class="collapse navbar-collapse" id="navbarCollapse">
                        <ul class="navbar-nav me-auto mb-2 mb-md-0">
                            <li class="nav-item">
                                <a class="nav-link active" aria-current="page" href="{$app-root}">Home</a>
                            </li>
                            <li class="nav-item">
                        ...
                        <span class="d-flex" role="search" id="userInfo">
                            <ul class="nav navbar-nav">
                                <li>
                                    <p class="navbar-btn">
                                        <xf:output xmlns="http://www.w3.org/2002/xforms" value="instance('i-user')//*:user"
    ...
    <xsl:template name="elementTemplate">
        ...
        <TEI xmlns="http://www.tei-c.org/ns/1.0">
            ...
            <xsl:for-each-group select="$globalSchemaDoc//descendant-or-self::tei:elementSpec | $localSchemaDoc//descendant-or-self::tei:elementSpec"
                group-by="@ident">
                <xsl:sort select="current-grouping-key()"/>
    ...
    <xsl:template name="xformElementSchemaInstance">
        ...
        <xsl:variable name="elementLabel">
            <xsl:choose>
                <xsl:when test="$elementRules/descendant-or-self::tei:local/descendant::tei:gloss[@xml:lang = $formLang][not(@type = 'tei')]">
                    <xsl:value-of
                        select="$elementRules/descendant-or-self::tei:local/descendant::tei:gloss[not(@type = 'tei')][@xml:lang = $formLang][1]"/>
                </xsl:when>
                ...
                <xsl:when test="$elementRules/descendant-or-self::tei:global/descendant::tei:gloss[not(@type = 'tei')][@xml:lang = $formLang]">
                    <xsl:value-of
                        select="$elementRules/descendant-or-self::tei:global/descendant::tei:gloss[not(@type = 'tei')][@xml:lang = $formLang][1]"/>
                </xsl:when>
        ...
        <xsl:variable name="maxOccur">
            <xsl:choose>
                ...
                <xsl:when test="$max != ''">
                    <xsl:choose>
                        <xsl:when test="$max castable as xs:integer">
                            <xsl:value-of select="$max"/>
                        </xsl:when>
                ...
                <xsl:when test="$parentElementRules/tei:content/tei:sequence[child::*[@key = $elementName]][@maxOccur]">
                    <xsl:choose>
                        <xsl:when test="$parentElementRules/tei:content/tei:sequence[child::*[@key = $elementName]]/@maxOccur = 'unbounded'"
                            >100</xsl:when>
                        <xsl:when test="$parentElementRules/tei:content/tei:sequence[child::*[@key = $elementName]]/@maxOccur != ''">
                            <xsl:choose>
                                <xsl:when
                                    test="$parentElementRules/tei:content/tei:sequence[child::*[@key = $elementName]]/@maxOccur castable as xs:integer">
        ...
        <xsl:variable name="minOccur">
            <xsl:choose>
                ...
                <xsl:when test="$min != ''">
                    <xsl:choose>
                        <xsl:when test="$min castable as xs:integer">
                            <xsl:value-of select="$min"/>
                        </xsl:when>
                        <xsl:otherwise>0</xsl:otherwise>
                    </xsl:choose>
                </xsl:when>
                <xsl:when test="$parentElementRules/tei:content/tei:sequence[child::*[@key = $elementName]][@minOccur]">
                    <xsl:choose>
        ...
        <xsl:element name="{$elementName}" namespace="http://www.tei-c.org/ns/1.0">
            ...
            <xsl:if
                test="$configDoc/descendant::*:lookup[@elementName = $elementName] and not($configDoc/descendant::*:subform[@formName = $subformName][@lookup = 'no'])">
                <lookup>
                    <xsl:copy-of select="$configDoc/descendant::*:lookup[@elementName = $elementName]/@*"/>
                    <xsl:attribute name="formURL"
                        select="concat('form.xq?form=forms/', $configDoc//formName/text(), '/lookup/', $elementName, '.xhtml')"/>
                </lookup>
            </xsl:if>
            <xsl:if
                test="$configDoc/descendant::*:popup[@elementName = $elementName] and not($configDoc/descendant::*:subform[@formName = $subformName][@lookup = 'no'])">
                <popup>
                    <xsl:copy-of select="$configDoc/descendant::*:popup[@elementName = $elementName]/@*"/>
                    <xsl:attribute name="formURL"
                        select="concat('form.xq?form=forms/', $configDoc/descendant::*:popup[@elementName = $elementName]/@formName, '/index.xhtml')"
                    />
                </popup>
            </xsl:if>
                    ...
                    <xsl:choose>
                        <xsl:when test="$formLang != ''">
                            <xsl:choose>
                                ...
                                <xsl:otherwise>
                                    <xsl:value-of select="$elementRules/descendant::tei:desc[1]"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </xsl:when>
                        <xsl:when test="$elementRules/descendant::tei:desc[@xml:lang = 'en']">
            ...
            <xsl:if test="$configDoc/descendant::*:subform[@formName = $subformName]/*:controlledValuesElements/*:element[@ident = $elementName]">
                <controlledValues xmlns="http://www.tei-c.org/ns/1.0">
                    <xsl:copy-of
                        select="$configDoc/descendant::*:subform[@formName = $subformName]/*:controlledValuesElements/*:element[@ident = $elementName]"
                        copy-namespaces="true"/>
                </controlledValues>
            </xsl:if>
                ...
                <xsl:if test="$childElements">
                    <child>
                        <xsl:for-each select="$childElements/child::*">
                            ...
                            <xsl:variable name="childLabel">
                                <xsl:choose>
                                    <xsl:when test="$elementRules/descendant-or-self::tei:local/descendant::tei:gloss[@xml:lang = $formLang]">
                                        <xsl:value-of
                                            select="$elementRules/descendant-or-self::tei:local/descendant::tei:gloss[@xml:lang = $formLang][1]"/>
                                    </xsl:when>
                                    ...
                                    <xsl:when test="$elementRules/descendant-or-self::tei:global/descendant::tei:gloss[@xml:lang = $formLang]">
                                        <xsl:value-of
                                            select="$elementRules/descendant-or-self::tei:global/descendant::tei:gloss[@xml:lang = $formLang][1]"/>
                                    </xsl:when>
                            ...
                            <xsl:variable name="max" select="@maxOccurs"/>
                            ...
                            <xsl:variable name="maxOccur">
                                <xsl:choose>
                                    ...
                                    <xsl:when test="$parentElementRules/tei:content/tei:sequence[child::*[@key = $elementName]][@maxOccur]">
                                        <xsl:choose>
                                            <xsl:when
                                                test="$parentElementRules/tei:content/tei:sequence[child::*[@key = $elementName]]/@maxOccur = 'unbounded'"
                                                >100</xsl:when>
                                            <xsl:when
                                                test="$parentElementRules/tei:content/tei:sequence[child::*[@key = $elementName]]/@maxOccur != ''">
                                                <xsl:choose>
                                                    <xsl:when
                                                        test="$parentElementRules/tei:content/tei:sequence[child::*[@key = $elementName]]/@maxOccur castable as xs:integer">
                                                        <xsl:value-of
                                                            select="$parentElementRules/tei:content/tei:sequence[child::*[@key = $elementName]]/@maxOccur"
                                                        />
                                                    </xsl:when>
                                                    <xsl:otherwise>100</xsl:otherwise>
                                                </xsl:choose>
                            ...
                            <xsl:variable name="minOccur">
                                <xsl:choose>
                                    ...
                                    <xsl:when test="$parentElementRules/tei:content/tei:sequence[child::*[@key = $elementName]][@minOccur]">
                                        <xsl:choose>
                                            <xsl:when
                                                test="$parentElementRules/tei:content/tei:sequence[child::*[@key = $elementName]]/@minOccur castable as xs:integer">
                                                <xsl:value-of
                                                    select="$parentElementRules/tei:content/tei:sequence[child::*[@key = $elementName]]/@minOccur"/>
                                            </xsl:when>
                                            <xsl:otherwise>0</xsl:otherwise>
                                        </xsl:choose>
                                    </xsl:when>
                            ...
                            <element ident="{$childName}" label="{$childLabel}" minOccurs="{$minOccur}" maxOccurs="{$maxOccur}"/>
                        </xsl:for-each>
                    </child>
                </xsl:if>
            </childElements>
