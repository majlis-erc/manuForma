...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:sc="http://www.ascc.net/xml/schematron" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:xi="http://www.w3.org/2001/XInclude" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:srophe="https://srophe.app" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xf="http://www.w3.org/2002/xforms" xmlns:local="http://syriaca.org/ns" version="3.0">
    ...
    <!-- Default config file. Use config parameter at run time to change.  -->
    <xsl:param name="config" select="base-uri()"/>
                            ...
                            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 border-bottom">
                                <h1 class="h2"><!--<xsl:value-of select="$configDoc//formTitle"/>-->
                                   Edit Record: <xf:output value="instance('i-rec')//*:titleStmt/*:title[1]" class="elementLabel"/>
                                    <xf:output value="concat('[',substring-after(instance('i-rec')//*:publicationStmt/*:idno[@type='URI'],'https://jalit.org/'),']')" class="elementLabel"/>
                                </h1>
                                    ...
                                    <xf:trigger class="btn btn-outline-secondary" appearance="minimal">
                                        ...
                                        <xsl:choose>
                                            <xsl:when test="contains($elementName,'/')">
                                                ...
                                                <xsl:variable name="stripConditions">
                                                    <xsl:choose>
                                                        <xsl:when test="contains($parent,'[')">
                                                            <xsl:value-of select="replace(substring-before($parent,'['),'tei:','')"/>
                                                        </xsl:when>
                                                        <xsl:otherwise><xsl:value-of select="replace($parent,'tei:','')"/></xsl:otherwise>
                                                    </xsl:choose>
                                                </xsl:variable>
                                                ...
                                                <xf:insert ev:event="DOMActivate" ref="parent::*" at="index('{$grpRepeatID}a')" origin="instance('i-{$subformName}-elementTemplate')/*[local-name() = '{$stripConditions}']" position="before"/>
                                            </xsl:when>
                                            <xsl:otherwise>
                                                <xsl:variable name="stripConditions">
                                                    <xsl:choose>
                                                        <xsl:when test="contains($elementName,'[')">
                                                            <xsl:value-of select="replace(substring-before($elementName,'['),'tei:','')"/>
                                                        </xsl:when>
                                                        <xsl:otherwise><xsl:value-of select="replace($elementName,'tei:','')"/></xsl:otherwise>
                                                    </xsl:choose>
                                                </xsl:variable>
                                                ...
                                                <xf:insert ev:event="DOMActivate" ref="." at="index('{$grpRepeatID}a')" origin="instance('i-{$subformName}-elementTemplate')/*[local-name() = '{$stripConditions}']" position="before"/>
                                            </xsl:otherwise>
                                        </xsl:choose>
                                    </xf:trigger>   
                                </div>
    ...
    <xsl:template name="attributeDisplay">
        ...
        <xf:repeat xmlns="http://www.w3.org/2002/xforms" ref="@*[local-name() != 'source']" class="attr-group">
            <div xmlns="http://www.w3.org/1999/xhtml" class="btn-group" role="group">
                <div class="input-group">
                    <!-- Controlled value -->
                    <xf:output class="restricted" xmlns="http://www.w3.org/2002/xforms" value=".[instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current()/parent::*)][1]//*:availableAtts/*:attDef[@ident = name(current())][@style='restricted']]">
<!--                        <xf:label ref="instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current()/parent::*)][1]//*:availableAtts/*:attDef[@ident = name(current())]/@attLabel"/>-->
                    </xf:output>
                    <!-- Attribute value -->
                    <xf:input xmlns="http://www.w3.org/2002/xforms" ref=".[not(instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current()/parent::*)][1]//*:availableAtts/*:attDef[@ident = name(current())]//*:valList)][not(instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current()/parent::*)][1]//*:availableAtts/*:attDef[@ident = name(current())][@style='restricted'])]" class="attVal"/>
                    <!-- If controlled attribute values in schemaConstraints file -->
                    <xf:select1 xmlns="http://www.w3.org/2002/xforms" ref=".[instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current()/parent::*)][1]//*:availableAtts/*:attDef[@ident = name(current())]//*:valList][not(instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current()/parent::*)][1]//*:availableAtts/*:attDef[@ident = name(current())][@style='restricted'])]" class="attVal">
                    ...
                    <xf:trigger xmlns="http://www.w3.org/2002/xforms" class="btn btn-outline-secondary btn-sm controls" appearance="full" ref=".[not(instance('i-{$subformName}-schemaConstraints')/*[local-name() = local-name(current()/parent::*)][1]//*:availableAtts/*:attDef[@ident = name(current())][@style='restricted'])]">
